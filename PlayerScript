...
var verified = {"qazwseer2":true}
function chat(e) {
if (e.message.startsWith("pcheck")) {
    if(verified[e.player.getName()]){
    var pstring = (e.message.replace("pcheck", "")).split(" ")
    for (var h = 0; h < pstring.length;h++) {
    if (e.player.world.getPlayer(pstring[h]) != null) {
    var player = e.player.world.getPlayer(pstring[h])
    var party = player.pixelmonData.party.getAll()
    var team = []
    var points = 0
    for(var i = 0; i < player.pixelmonData.party.countAblePokemon(); i++) {
        ...
        if (party[i].form == 2 && party[i].getBaseStats().pixelmonName.toLowerCase() == "kyurem" ) {
            team[i] = team[i]+"-white"
        }
        if (party[i].form == 1 && party[i].getBaseStats().pixelmonName.toLowerCase() == "kyurem" ) {
            team[i] = team[i]+"-black"
        }
    }
    e.player.message("§c§l"+team)

    for (var i = 0; i < team.length; i++) {
        if ((team[i] in PointList) == true) {
            e.player.message("§e§l"+team[i]+"§a§l - §b§l" + PointList[team[i]] + "§a§l очков")
            points =points+ PointList[team[i]]
        }
        if ((team[i] in PointList) == false) {
            e.player.message("§e§l"+team[i]+"§a§l - §b§l0§a§l очков или неверное название покемона")
        }
    }
    e.player.message("§6§lВсего - §4§l" + points + "§6§l очков у §c§l"+player.getName())
    }
    }
    e.setCanceled(true)
    }
